...
export class FilesController {
  ...
  async download(
    ...
    // Use X-Forwarded-Host if available (when behind proxy), otherwise fallback to Host header
    const host = (req as any).headers?.['x-forwarded-host'] || (req as any).headers?.host || (req as any).hostname;
    const url = await this.filesService.getDownloadUrl(req.user.userId, id, host);
    return { url };
  }
  ...
  async getThumbnails(
    ...
    const urls = await this.filesService.getThumbnailUrls(req.user.userId, body.ids);
    return urls;
  }

  @Post('public/thumbnails')
  async getPublicThumbnails(
    @Body() body: { ids: string[] },
  ) {
